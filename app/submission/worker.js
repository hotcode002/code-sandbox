...
/**
 * Initialize a response variable that will hold all the data to be sent back
 * like the compilation and execution results etc.
 */
const response = {};
try {
    const box_id = workerData.box_id;
    const language = workerData.language;
    const source_code = workerData.source_code;
    const timer = workerData.timer;
    ...
    if (languageDefaultOptions.compile) {
        ...
        console.log(compile.stderr.toString());
